...
class TrainDataLoader(object):
    def __init__(self, all_utterances, labels, word_dict, speakers, name='train', add_noise=False, batch_size=constant.batch_size):
    ...
    def __getitem__(self, key):
        ...
        padded_speakers = padded_speakers.type(torch.DoubleTensor)
        ...
        return new_utterance_num_numpy, utterance_sequence_length, conversation_lengths, padded_labels, padded_speakers
    ...
    def convert_to_tensors_label(self, batch, batch_size, conversation_lengths):
        ...
        if not torch.cuda.is_available():
            ...
            for i in range(len(batch)):
                new_batch[i, :conversation_lengths[i]] = torch.cuda.LongTensor(batch[i])
